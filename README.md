...
| Veilshroud | Marks a position, 1.5s delay, lightning strikes only if the target is still there | Marks the nearest enemy's position, 0.75s delay, a bolt strikes that spot, only damaging whatever is still standing there: 320 + 17% EP true damage (Phantom Strike). |
| Egregor | Windup tracking the target, then a 180° tentacle-whip slash reaching however far the target is | 0.5s windup locking the nearest enemy's direction, then a real 180° arc slash centered on the player reaching up to the full screen diagonal: 260 + 14% EP (Null Slash). |
| Dargruel | An expanding shockwave ring covering the full screen (`spawnBossShockwave`), not an instant flat hit | An expanding ring covering the full screen diagonal over 0.7s, damaging each enemy the instant it passes through them: 190 + 11% EP (Root Shockwave). |
