...
**Tesla Coils** — After 5 seconds, each linked pair of orbs merges into a Tesla Coil (max 4 total). Each coil has a radius-200 aura that slows enemies and strips **5% DR and 100 flat DR** from them (a target with less than that to lose takes the shortfall as bonus damage: 5% per missing DR point, 1:1 for flat). Every ~0.42 seconds (0.5s with a +20% fire rate) each coil picks up to **5 different enemies** inside the aura, charges for ~0.08 seconds, then fires a homing bolt at each one, ~0.08 seconds apart, for **25% ATK piercing** damage (Circuit Engineer sigil: +30%). Piercing here is literal: a bolt doesn't stop at its assigned target, it keeps homing onto the next untried enemy in range and can land on several targets before its 2.2s life runs out or it flies off-screen. A bolt hit also **slows** its target 32% for 1.5 seconds (a fixed strength that only refreshes the timer, never stacks; CC-immune enemies ignore it). Bolts do not trigger Glory for Justice's Chain Lightning. Coils have no health. Instead every bolt a coil fires adds one **stack** (max 5), shown as a 5-cell bar above the coil; the whole combo shares one 1.5-second timer that every new bolt refreshes, so sustained fire keeps building instead of the oldest stack quietly expiring mid-combo, and the whole bar only lapses back to 0 after 1.5s with no bolt fired at all. Each stack adds **+8% fire rate** to that coil and **+5% ATK** to every bolt hit, all cumulative. Once the coil holds 5 stacks, its next bolt is **empowered**: it deals an extra **18% ATK plus 10% of the target's lost HP plus 2.25% of the target's Max HP**, roots the target for **0.75s**, and resets the stacks to 1 (the empowered bolt counts as the first stack of the next cycle). Enemy bullets no longer hurt a coil, and each enemy bullet that flies into an aura has a **25% chance to vanish** (rolled once per bullet), on top of being slowed. When several coils slow the same enemy (overlapping auras or bolts from different coils), the first slow lands in full and each extra coil's slow is **40% less effective**. A coil that has fired 500 bolts self-destructs.

**Detonation** — When the 30-second duration ends, all remaining orbs and coils explode. Each blast deals **2.3% ATK** in a large area (Circuit Engineer sigil: +30%).
