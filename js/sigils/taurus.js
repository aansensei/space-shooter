...
SIGIL_DEFS.taurus = {
        ...
        buffs: [
            ...
            { id: 'hiep_luc', name: 'Squad Synergy', type: 'ATK', typeC: '#ef4444',
              desc: 'Tank: absorb meter fills from squad damage and from damage it soaks (30%) off real Sentinels\' hits; at 100 fires a piercing SentinelBlade sweep (40 + 4% Max HP true dmg per enemy) and heals itself 20 HP. Support: heals the 2 lowest-HP% targets across both the squad and real Sentinels for 50 HP each (never the player). Marksman: auto-fires a piercing arrow every 0.3s (75 + 5% Max HP). Mage: orbiting crystal flies to the nearest enemy roughly every 1s and detonates an arcane AoE zone (3s, 50 + 4% Max HP/tick); Role Match: every 4th cast (running count) is empowered, lighting the caster up with a brighter bolt and impact, and its zone tags anything it ticks with Soul Reaver for 3.5s.' },
        ]
};
...
SIGIL_I18N_VI.taurus = { name: 'Kim Ngưu', element: 'Thổ', buffs: {
        ...
        hiep_luc: { name: 'Hiệp Lực Tổ Đội', desc: 'Tank: thanh hấp thụ đầy từ sát thương đội hình và từ phần dame nó gánh hộ (30%) mỗi khi Vệ Binh thật ăn đòn; đầy 100 → phóng lưỡi chém xuyên SentinelBlade (40 + 4% Max HP sát thương chuẩn mỗi kẻ địch) kèm tự hồi 20 HP. Support: hồi máu cho 2 mục tiêu %HP thấp nhất trong CẢ đội hình lẫn Vệ Binh thật (không phải người chơi), mỗi người 50 HP. Marksman: tự động bắn mũi tên xuyên mỗi 0.3s (75 + 5% Max HP). Mage: ngọc vệ tinh bay đến kẻ địch gần nhất khoảng mỗi 1s và nổ vùng arcane AoE (3s, 50 + 4% Max HP/tick); Role Match: cứ mỗi lần phóng thứ 4 (đếm dồn) được cường hóa, pháp sư sáng rực lên với viên phép và cú nổ mạnh hơn, và vùng phép đó gắn Soul Reaver 3.5s cho mọi kẻ địch nó tick trúng.' },
}};
