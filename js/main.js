...
function playerTakesHit(attacker) {
    ...
    // passes through with no damage). Sentinels get the matching check in
    // dealDamage, entities/core.js.
    if (window._greatSageStealthEnd && performance.now() < window._greatSageStealthEnd) {
        return false;
    }
...
function update(rawDeltaTime) {
    ...
    // ever accumulate faster than they clear (e.g. mobile's inflated
    // canvas resolution means a bullet travels further, so survives more
    // frames, before crossing the off-screen check below), the array can
    // grow unbounded. Every extra bullet costs a full collision scan, so
    // this is the kind of cost the quality-tier system can never reduce no
    // matter how low it goes. Trim the oldest excess (front of the array,
...
// back to 11, matching the original tuning (waves below that are short
