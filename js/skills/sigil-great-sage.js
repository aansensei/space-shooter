        ...
        if (fx.type === 'raphael') {
                ...
                fx.angles.forEach(angle => {
                    ...
                    for (const enemy of enemies) {
                        ...
                        if (distToSegment(enemy, lineStart, lineEnd) < (enemy.size || 20) + 15) {
                            dealDamage(enemy, { damage: 0.33 * player.atk * fx.comboMult, percentDamage: 0.035, _statSrc: 'Great Sage: Lumen Nova' }); // docs/combat-scaling-rebalance.md Part 5
                        }
                    }
                });
                ...
                fx.points.forEach(pt => {
                    for (const enemy of enemies) {
                        ...
                        if (Math.hypot(enemy.x - pt.x, enemy.y - pt.y) < (enemy.size || 20) + 30) {
                            dealDamage(enemy, { damage: 0.48 * player.atk * fx.comboMult, percentDamage: 0.035, isTrueDamage: true, _statSrc: 'Great Sage: Phantom Strike' }); // docs/combat-scaling-rebalance.md Part 5
                        }
                    }
                ...
                if (fx.timer >= fx.dur) {
                    ...
                    for (const enemy of enemies) {
                        ...
                        if (Math.abs(dA) <= Math.PI / 2) {
                            dealDamage(enemy, { damage: 0.39 * player.atk * fx.comboMult, percentDamage: 0.035, _statSrc: 'Great Sage: Null Slash' }); // docs/combat-scaling-rebalance.md Part 5
                        }
                    }
            ...
            for (const enemy of enemies) {
                ...
                if (Math.hypot(enemy.x - fx.x, enemy.y - fx.y) <= curRadius + 20) {
                    ...
                    if (enemy.type.startsWith('enemy_bullet')) {
                        ...
                        dealDamage(enemy, { damage: 0.284 * player.atk * fx.comboMult, percentDamage: 0.035, _statSrc: 'Great Sage: Root Shockwave' }); // docs/combat-scaling-rebalance.md Part 5
                    }
                }
            }
            ...
            if (fx.phase === 'warn' && fx.timer >= fx.warnDur) {
                ...
                for (const enemy of enemies) {
                    ...
                    if (d < 0.15) {
                        fx.hitEnemies.push(enemy);
                        dealDamage(enemy, { damage: 0.30 * player.atk * fx.comboMult, percentDamage: 0.035, _statSrc: 'Great Sage: Perseverance Sweep' }); // docs/combat-scaling-rebalance.md Part 5
                    }
                }
