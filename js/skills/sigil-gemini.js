    ...
    for (let i = window._shadowOrbs.length - 1; i >= 0; i--) {
        ...
        for (const enemy of enemies) {
            ...
            if (Math.hypot(enemy.x - orb.x, enemy.y - orb.y) < enemy.size / 2 + (orb.isLarge ? 15 : 10)) {
                ...
                if (orb.isLarge) {
                    // Small target-Max-HP term kept deliberately (per AanSensei) on the large orb only.
                    dealDamage(enemy, { damage: 0.27 * player.atk, percentDamage: 0.025, applySoulReaver: true, _noHitSfx: true, _statSrc: 'Shadow Twin' });
                } else {
                    dealDamage(enemy, { damage: 0.112 * player.atk, applySoulReaver: true, _noHitSfx: true, _statSrc: 'Shadow Twin' });
                }
            ...
            enemies.forEach(enemy => {
                ...
                if (enemy.y < player.y && Math.abs(enemy.x - laserX) < 100 / 2) {
                    ...
                    if (enemy.type === 'marchosias' && enemy.arcBarrier && enemy.arcBarrier.hp > 0) {
                        const _src = { damage: 0.525 * player.atk, percentDamage: 0.025, isPiercing: true, _barrierPiercing: true, _statSrc: 'Sigil: Mirror Laser' };
                        ...
                        dealDamage(enemy, { damage: 0.525 * player.atk, percentDamage: 0.025, isPiercing: true, _statSrc: 'Sigil: Mirror Laser' });
                    }
                }
            });
        }
