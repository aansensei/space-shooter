    ...
    taurus: {
        ...
        buffs: [
            { id: 'doi_hinh_chien', name: 'Yuusha Party', type: 'SPEC', typeC: '#f59e0b',
              desc: 'Summon a squad of 4 Sentinels that follow in formation above the player, one of each role (Tank 300 HP, Support/Marksman/Mage 200 HP each). Each member has its own HP bar and role-specific weapon, respawns 8s after falling (same role), and shares every buff real Sentinels get (Blessing, Gaia Protection/Barrier, Lunar Aegis, Last Stand, Glory for Justice). Enemy bullets passing near the squad can strike a member: Tanks draw ~75% of it. Piercing enemy attacks that would cost the player a life instead strike every living squad member at once, each taking 1.5x that same attack\'s real-Sentinel damage (non-piercing hits are unaffected). Every 5s, if no real Sentinels remain, 2 are spawned back in (2s cooldown).' },
            { id: 'hiep_luc', name: 'Squad Synergy', type: 'ATK', typeC: '#ef4444',
              desc: 'Tank: absorb meter fills from squad damage and from damage it soaks (15%) off real Sentinels\' hits; at 100 fires a piercing SentinelBlade sweep (40 + 4% EP true dmg per enemy) and heals itself 5 HP. Support: heals the 2 lowest-HP% targets across both the squad and real Sentinels for 50 HP each (never the player). Marksman: auto-fires a piercing arrow every 0.3s (75 + 5% EP). Mage: orbiting crystal flies to the nearest enemy roughly every 1s and detonates an arcane AoE zone (3s, 50 + 4% EP/tick).' },
        ]
    },
    gemini: {
